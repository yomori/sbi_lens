...
def get_samples_and_scores_fixedcosmo(
    model,
    key,
    batch_size=64,
    score_type="density",
    ...
    if thetas is None:
        ...
        thetas = get_params(keys)
        thetas = jnp.array([[0.2664,0.0492,0.831,0.6727,0.9645,-1.0]])

    return jax.vmap(jax.value_and_grad(log_prob_fn, has_aux=True))(thetas, keys)



def get_samples_and_scores_fixedcosmo2(
    model,
    key,
    batch_size=64,
    score_type="density",
...
def _lensingPS2(map_size, sigma_e, a, b, z0, gals_per_arcmin2, ell, nbins):
    # Field parameters
    f_sky = map_size**2 / 41_253
    nz = jc.redshift.smail_nz(a, b, z0, gals_per_arcmin2=gals_per_arcmin2)
    nz_bins = subdivide(nz, nbins=nbins, zphot_sigma=0.05)
    ...
    cosmo = jc.Planck15(Omega_c=omega_c, Omega_b=omega_b, h=h_0, n_s=n_s, sigma8=sigma_8, w0=w_0)
...
def get_reference_sample_posterior_power_spectrum2(
    run_mcmc=False,
    N=256,
    map_size=10,
    gals_per_arcmin2=27,
